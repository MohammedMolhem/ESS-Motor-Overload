 ...
 *
 * Neyman-Pearson: fixed Pfa on fused energy ratio (F approximation); defaults biased
 *   toward accuracy (finer LS search, more harmonics/kurtogram bands, larger subspace).
 *
 * Spectral resolution: one FFT frame spans T = N/fs seconds with bin spacing Δf = fs/N.
 *   (2048 @ 5 kHz → ~2.44 Hz, 0.4096 s; 4096 → ~1.22 Hz, 0.8192 s — high-accuracy build.)
 * ============================================================================
 */

#define FP_SAMPLES_PER_PHASE           4096u
...
/* USB: Fourier + MUSIC at native one-sided FFT resolution (one column per DFT bin, Δf=fs/N). */
 ...
 * ASCII: key=value lines (CR/LF terminated)
 *   PING GET RPM= SLIP= BPFO= BPFI= FTF= BSF= FTH= WTH= LINE= (GET adds DATE= TIME=)
 ...
 *   GRAPHDATA=native 4096 pts/phase + 2048 FFT bins for plots; FFTCSV ... (one technique each)
 */
...
static float g_usb_export_fft[USB_EXPORT_GRID_BINS];
static float g_usb_export_music[USB_EXPORT_GRID_BINS];
static float g_usb_export_cyclic2[USB_EXPORT_GRID_BINS];
static float g_usb_export_sk[USB_EXPORT_GRID_BINS];
static float g_cyclic2_psq[FP_HALF_FFT];
static float g_phase_a_csv[FP_SAMPLES_PER_PHASE];
static float g_phase_b_csv[FP_SAMPLES_PER_PHASE];
static float g_phase_c_csv[FP_SAMPLES_PER_PHASE];
...
static void USB_Send_Ack(uint8_t cmd, uint8_t status)
{
  ...
  (void)CDC_Transmit_FS(usb_cdc_tx_buf, 6u);
}
...
static void USB_Send_AllParams(void)
{
  ...
  float vals[8] = {
    g_params.motor_rpm,
    g_params.motor_slip,
    g_params.bpfo_coefficient,
    g_params.bpfi_coefficient,
    g_params.ftf_coefficient,
    g_params.bsf_coefficient,
    g_params.fault_threshold,
    g_params.warning_threshold
  };
  ...
  usb_cdc_tx_buf[idx++] = 0x55;
  usb_cdc_tx_buf[idx++] = 0xAA;
  (void)CDC_Transmit_FS(usb_cdc_tx_buf, (uint16_t)idx);
}
...
static void USB_Send_TextLine(const char *s)
{
  ...
  usb_cdc_tx_buf[n] = '\0';
  (void)CDC_Transmit_FS(usb_cdc_tx_buf, (uint16_t)n);
}
...
static void USB_Process_TextLine(uint8_t *buf, uint32_t len)
{
  ...
  if (eq != NULL)
  {
    ...
    const float v = strtof(eq + 1, NULL);

    if (strcmp(key, "rpm") == 0) {
      if (Validate_RPM(v)) { g_params.motor_rpm = v; USB_Send_TextLine("OK\r\n"); }
      else USB_Send_TextLine("ERR RPM range (0..50000)\r\n");
      return;
    }
    if (strcmp(key, "slip") == 0) {
      if (Validate_Slip(v)) { g_params.motor_slip = v; USB_Send_TextLine("OK\r\n"); }
      else USB_Send_TextLine("ERR SLIP range (0..1)\r\n");
      return;
    }
    if (strcmp(key, "bpfo") == 0) {
      if (Validate_Coefficient(v)) { g_params.bpfo_coefficient = v; USB_Send_TextLine("OK\r\n"); }
      else USB_Send_TextLine("ERR BPFO range (0..100)\r\n");
      return;
    }
    if (strcmp(key, "bpfi") == 0) {
      if (Validate_Coefficient(v)) { g_params.bpfi_coefficient = v; USB_Send_TextLine("OK\r\n"); }
      else USB_Send_TextLine("ERR BPFI range (0..100)\r\n");
      return;
    }
    if (strcmp(key, "ftf") == 0) {
      if (Validate_Coefficient(v)) { g_params.ftf_coefficient = v; USB_Send_TextLine("OK\r\n"); }
      else USB_Send_TextLine("ERR FTF range (0..100)\r\n");
      return;
    }
    if (strcmp(key, "bsf") == 0) {
      if (Validate_Coefficient(v)) { g_params.bsf_coefficient = v; USB_Send_TextLine("OK\r\n"); }
      else USB_Send_TextLine("ERR BSF range (0..100)\r\n");
      return;
    }
    if (strcmp(key, "fth") == 0) {
      if (Validate_Threshold(v)) { g_params.fault_threshold = v; USB_Send_TextLine("OK\r\n"); }
      else USB_Send_TextLine("ERR FTH range (0..1000)\r\n");
      return;
    }
    if (strcmp(key, "wth") == 0) {
      if (Validate_Threshold(v)) { g_params.warning_threshold = v; USB_Send_TextLine("OK\r\n"); }
      else USB_Send_TextLine("ERR WTH range (0..1000)\r\n");
      return;
    }
    if (strcmp(key, "line") == 0) {
      if (isfinite(v) && v > 1.0f && v < 400.0f) g_supply_line_hz = v;
  ...
  {
    USB_Send_TextLine(
      "PING GET REPORT FULLREPORT GRAPHDATA PHASE3 PHASECSV FFTCSV MUSICCSV ESPRITCSV CYCLIC2CSV SKCSV WAVELETCSV\r\n"
      "RPM= SLIP= BPFO= BPFI= FTF= BSF= FTH= WTH= LINE= DATE TIME SAVE LOAD DEFAULT SAVEADC\r\n"
      "CALIB SAVEBASE LOADBASE CLEARBASE CALIBST SAVESTST LOADSTST CLEARSTST\r\n"
      "FULLREPORT=scalars+spectral+GRAPHDATA; GRAPHDATA=ABC+SEQ+FFT+envelope\r\n");
    return;
  }
...
#define U_D(x, d)                                                              \
    ...
    U_SN("%s", "RPM=");   U_D(g_params.motor_rpm, 3u);
    U_SN("%s", " SLIP="); U_D(g_params.motor_slip, 4u);
    U_SN("%s", " BPFO="); U_D(g_params.bpfo_coefficient, 4u);
    U_SN("%s", " BPFI="); U_D(g_params.bpfi_coefficient, 4u);
    U_SN("%s", " FTF=");  U_D(g_params.ftf_coefficient, 4u);
    U_SN("%s", " BSF=");  U_D(g_params.bsf_coefficient, 4u);
    U_SN("%s", " FTH=");  U_D(g_params.fault_threshold, 3u);
    U_SN("%s", " WTH=");  U_D(g_params.warning_threshold, 3u);
    U_SN("%s", " LINE="); U_D(g_supply_line_hz, 2u);
    {
      uint16_t cy;
...
get_tx:
    *p = '\0';
    (void)CDC_Transmit_FS(usb_cdc_tx_buf, (uint16_t)strlen((char *)usb_cdc_tx_buf));
    return;
  }
...
static void USB_Process_Received_Data(void)
{
  ...
  switch (cmd)
  {
    ...
    case CMD_SET_RPM:
      if (g_usb_rx_len >= 5u) {
        float v = usb_read_f32_le(&g_usb_rx_buf[1]);
...
static float wav_buf_a[FP_FFT_SIZE];
static float wav_buf_b[FP_FFT_SIZE];
...
static void Window_Apply(float *sig, uint32_t n)
{
  for (uint32_t i = 0; i < n; i++) sig[i] *= window_hamming[i];
}
...
static void Bearing_CalcFaultFundamentals(float *bpfo, float *bpfi, float *bsf, float *ftf)
{
  const float fr = compute_shaft_frequency_hz();
  *bpfo = g_params.bpfo_coefficient * fr;
  *bpfi = g_params.bpfi_coefficient * fr;
  *ftf  = g_params.ftf_coefficient  * fr;
  *bsf  = g_params.bsf_coefficient  * fr;
}
...
{
  const float w = 2.0f * 3.14159265358979323846f * f_hz / fs;
  float scc = 0.f, scs = 0.f, sss = 0.f, sxc = 0.f, sxs = 0.f;
  for (uint32_t n = 0; n < N; n++) {
    const float c = cosf(w * (float)n);
    const float s = sinf(w * (float)n);
    scc += c * c; scs += c * s; sss += s * s;
    sxc += x[n] * c; sxs += x[n] * s;
  }
  const float det = scc * sss - scs * scs;
  if (fabsf(det) < 1e-24f) { *out_a = 0.f; *out_b = 0.f; return; }
  *out_a = (sxc * sss - sxs * scs) / det;
  *out_b = (scc * sxs - scs * sxc) / det;
}
...
{
  const float w = 2.0f * 3.14159265358979323846f * f_line_hz / fs;
  float scc = 0.f, scs = 0.f, sss = 0.f, sxc = 0.f, sxs = 0.f;
  for (uint32_t n = 0; n < N; n++) {
    const float c = cosf(w * (float)n), s = sinf(w * (float)n);
    scc += c * c; scs += c * s; sss += s * s;
    sxc += x[n] * c; sxs += x[n] * s;
  }
  const float det = scc * sss - scs * scs;
  if (fabsf(det) < 1e-24f) return;
  const float A = (sxc * sss - sxs * scs) / det;
  const float B = (scc * sxs - scs * sxc) / det;
  for (uint32_t n = 0; n < N; n++) {
    const float wn = w * (float)n;
    x[n] -= A * cosf(wn) + B * sinf(wn);
  }
}
...
/* ============================================================================
 * Main bearing fault detection (runs on WINDOWED data)
 * NOTE: Stator_Winding_Analyze() is NO LONGER called here.
 *       It is called in main() BEFORE windowing.
 * ============================================================================
 */
...
static void Fault_Detect_Bearing(void)
{
  ...
  Bearing_CalcFaultFundamentals(&bpfo, &bpfi, &bsf, &ftf);
  ...
  /* Neyman-Pearson + EMA + CUSUM thresholding */
  if (ls_base_cnt > 1u && ls_fault_cnt > 0u) {
    const float d1 = 2.0f * (float)ls_fault_cnt;
    const float d2 = 2.0f * (float)ls_base_cnt;
    const float gamma_np = fdist_inv_cdf(1.0f - FP_NP_PFA, d1, d2);
    const float decide = fmaxf(g_fault.fault_index_ema, g_fault.fault_index);
    g_fault.fault_level = (decide > gamma_np || g_fault.cusum_score > FP_CUSUM_ALARM) ? 2u : 0u;
    if (g_fault.fault_level == 0u &&
        (g_fault.fault_index_ema > g_params.warning_threshold ||
  ...
  /* USB plot buffers: Fourier on cleaned mean residual; cyclic2 |FFT(x^2)|; MUSIC pseudospectrum */
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc)
{
   ...
   * this frame before the main loop runs (~0.82 s @ 5 kHz x 4096). */
...
static void USB_Send_ReportScalarsFull(void)
{
  ...
  *p = '\0';
  ...
  usb_tx_append_sn(&p, pe, " BPFO="); usb_tx_append_d(&p, pe, (double)g_params.bpfo_coefficient, 4u);
  usb_tx_append_sn(&p, pe, " BPFI="); usb_tx_append_d(&p, pe, (double)g_params.bpfi_coefficient, 4u);
  usb_tx_append_sn(&p, pe, " BSF="); usb_tx_append_d(&p, pe, (double)g_params.bsf_coefficient, 4u);
  usb_tx_append_sn(&p, pe, " FTF="); usb_tx_append_d(&p, pe, (double)g_params.ftf_coefficient, 4u);
...
{
  if (!g_phase_csv_ready) {
    USB_Send_TextLine("ERR FULLREPORT wait for capture (one frame ~0.82s)\r\n");
    return;
  }
...
static void USB_Send_Spectral_MetaAndModelIdx(const char *mode)
{
  ...
  usb_tx_append_sn(&p, pe, " SPEC_BINS=%u\r\n", (unsigned)USB_EXPORT_GRID_BINS);
  *p = '\0';
  USB_Tx_WaitSend((uint16_t)strlen(p0));
...
static void USB_Send_Spectral_Section_FourierCsv(void)
{
  ...
  for (uint32_t base = 0u; base < USB_EXPORT_GRID_BINS; base += USB_EXPORT_CSV_ROW_PAIRS) {
    p = p0;
    usb_tx_append_sn(&p, pe, "%s", "[FOURIER_CSV] ");
    for (uint32_t j = 0u; j < USB_EXPORT_CSV_ROW_PAIRS && base + j < USB_EXPORT_GRID_BINS; j++) {
      ...
      usb_tx_append_sn(&p, pe, ",");
      usb_tx_append_d(&p, pe, (double)g_usb_export_fft[i], 6u);
    }
    usb_tx_append_sn(&p, pe, "%s", "\r\n");
    *p = '\0';
    USB_Tx_WaitSend((uint16_t)strlen(p0));
...
static void USB_Send_Spectral_Section_Cyclic2Csv(void)
{
  ...
  for (uint32_t base = 0u; base < USB_EXPORT_GRID_BINS; base += USB_EXPORT_CSV_ROW_PAIRS) {
    p = p0;
    usb_tx_append_sn(&p, pe, "%s", "[CYCLIC2_CSV] ");
    for (uint32_t j = 0u; j < USB_EXPORT_CSV_ROW_PAIRS && base + j < USB_EXPORT_GRID_BINS; j++) {
      const uint32_t i = base + j;
      const float f_hz = ((float)i + 0.5f) * (FP_SAMPLE_RATE_HZ * 0.5f) / (float)USB_EXPORT_GRID_BINS;
      if (j > 0u) usb_tx_append_sn(&p, pe, "%s", ";");
      usb_tx_append_d(&p, pe, (double)f_hz, 3u);
      usb_tx_append_sn(&p, pe, ",");
      usb_tx_append_d(&p, pe, (double)g_usb_export_cyclic2[i], 6u);
    }
    usb_tx_append_sn(&p, pe, "%s", "\r\n");
    *p = '\0';
    USB_Tx_WaitSend((uint16_t)strlen(p0));
...
static void USB_Send_FaultSummary(void)
{
  ...
  *p = '\0';
  (void)CDC_Transmit_FS(usb_cdc_tx_buf, (uint16_t)strlen((char *)usb_cdc_tx_buf));
  HAL_Delay(2); /* wait for USB FS to finish before reusing buffer */
...
int main(void)
{
  ...
  while (1)
  {
    ...
    if (g_all_ready)
    {
       ...
       * Must run BEFORE Window_Apply so Fortescue phasor
       * magnitudes (I0, I1, I2) are accurate, not attenuated
       * by Hamming window (~54% bias).
       * ============================================================
       */
      Stator_Winding_Analyze(g_supply_line_hz);

      /* ============================================================
       * BEARING FAULT DETECTION — on WINDOWED data
       * Hamming window reduces spectral leakage for LS, MUSIC,
       * Teager, kurtogram, and wavelet paths.
       * ============================================================
       */
      Window_Apply(phase_a, FP_SAMPLES_PER_PHASE);
      Window_Apply(phase_b, FP_SAMPLES_PER_PHASE);
      Window_Apply(phase_c, FP_SAMPLES_PER_PHASE);
