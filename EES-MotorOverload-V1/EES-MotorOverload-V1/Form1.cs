    ...
    public partial class Form1 : Form
    {
        ...
        private async Task SelectTechnique(SpectralTechnique tech)
        {
            ...
            if (_comm != null && _comm.IsConnected)
            {
                if (!UseTextProtocol)
                {
                    LogUI("H750 binary firmware: spectral REPORT not available — refresh live metrics", Color.Orange);
                    TelemetryData m = await _comm.GetMetrics();
                    if (m != null)
                        Comm_OnTelemetry(m);
        ...
        private void BuildSettingsTab()
        {
            ...
            {
                Text = "Text Protocol (legacy V6 firmware)",
        ...
        private async void BtnConnect_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                MessageBox.Show(
                    ...
                    "  - H750 firmware flashed (D:\\last3\\main.c)\n" +
                    "  - Correct COM port selected\n" +
                    "  - Binary protocol (default) — enable Text Protocol only for legacy V6 firmware",
                    "Connection Failed",
                    MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        ...
        private async void BtnSendAll_Click(object sender, EventArgs e)
        {
            ...
            int expected = UseTextProtocol ? 8 : 9;
            if (UseTextProtocol && p.SupplyLineHz > 1.0f && p.SupplyLineHz < 400.0f)
                expected = 9;
            else if (!UseTextProtocol && (p.SupplyLineHz <= 10f || p.SupplyLineHz >= 400f))
                expected = 8;
        ...
        private async void BtnReadAll_Click(object sender, EventArgs e)
        {
            ...
            LogUI("Reading parameters (" + (UseTextProtocol ? "text" : "binary H750") + ")...", Color.Cyan);

            MotorParameters p = UseTextProtocol
                ? await _comm.GetAllParametersText()
                : await _comm.GetAllParameters();
        ...
        private async void BtnSaveAdc_Click(object sender, EventArgs e)
        {
            ...
            LogUI(ok ? "ADC snapshot save requested — saves on next cycle"
                     : "ADC snapshot: H750 uses automatic raw logging (see firmware raw_log_interval_s)",
                  ok ? Color.LimeGreen : Color.Orange);
        ...
        private async void BtnLoadFlash_Click(object sender, EventArgs e)
        {
            ...
            if (ok)
            {
                LogUI("Loaded from SPI flash — reading back...", Color.LimeGreen);
        ...
        private void Comm_OnTelemetry(TelemetryData t)
        {
            ...
            try
            {
                this.BeginInvoke(new Action(delegate
                {
                    if (t.IsBinaryMetrics)
                    {
                        lblBpfoHz.Text = "Total RMS: " + t.TotalRms.ToString("F4");
                        lblBpfiHz.Text = "Imbalance: " + t.ImbalancePct.ToString("F1") + "%";
                        lblBsfHz.Text = "Motor status: " + t.MotorStatus;
                        if (t.RmsA > 0f || t.RmsB > 0f || t.RmsC > 0f)
                            lblFtfHz.Text = "Ia/Ib/Ic: " + t.RmsA.ToString("F3") + " / " +
                                t.RmsB.ToString("F3") + " / " + t.RmsC.ToString("F3");
                        else
                            lblFtfHz.Text = "—";

                        lblInner.Text = "Fault index: " + t.FaultIndex.ToString("F4");
                        lblFTFAlarm.Text = "Imbalance: " + t.ImbalancePct.ToString("F1") + "%";
                        lblBSFAlarm.Text = "Total RMS: " + t.TotalRms.ToString("F4");
                        lblOuter.Text = "Motor status: " + t.MotorStatus;
                    }
                    else
                    {
        ...
        private void UpdateAlarmMonitorDashboard(TelemetryData t)
        {
            ...
            lblAlarmStatusBearing.Text = t.IsBinaryMetrics
                ? "Motor: " + t.FaultLevelString +
                  " | Fault index " + t.FaultIndex.ToString("F4") +
                  " | Total RMS " + t.TotalRms.ToString("F4") +
                  " | Imbalance " + t.ImbalancePct.ToString("F1") + "%"
