...
{
    // =========================================================================
    // COMMAND CONSTANTS — match D:\last3\main.c (WeAct STM32H750VBT6, USB CDC)
    // =========================================================================

    public static class STM32Commands
    {
        public const byte CMD_SET_RATED_CURRENT = 0x01;
        public const byte CMD_SET_LINE_FREQ = 0x02;
        public const byte CMD_SET_POLES = 0x03;
        ...
        public const byte CMD_SET_RAW_INTERVAL = 0x0C;

        public const byte CMD_GET_ALL_PARAMS = 0x20;
        public const byte CMD_GET_METRICS = 0x21;
        public const byte CMD_GET_FAULT_DETAIL = 0x23;
        ...
        public const byte CMD_GET_FLASH_STATUS = 0x33;

        public const byte CMD_READ_SUM_LOG = 0x40;
        public const byte CMD_ERASE_SUM_LOG = 0x41;
        public const byte CMD_GET_SUM_LOG_INFO = 0x42;
        public const byte CMD_READ_RAW_CHUNK = 0x50;
        public const byte CMD_ERASE_RAW_LOG = 0x51;
        public const byte CMD_GET_RAW_LOG_INFO = 0x52;

        /// <summary>Auto telemetry frame (BuildFrame cmd 0xF0).</summary>
        public const byte CMD_TELEMETRY = 0xF0;
        public const byte CMD_PING = 0xFE;

        // Legacy text-only / older host builds (not used on last3 firmware)
        public const byte CMD_GET_ALL_PARAMS_LEGACY = 0x80;
    ...
    public class MotorParameters
    {
        ...
        public ushort MotorPoles { get; set; }
    ...
    public class TelemetryData
    {
        ...
        public float RmsA { get; set; }
        public float RmsB { get; set; }
        public float RmsC { get; set; }
        public float ImbalancePct { get; set; }
        public byte MotorStatus { get; set; }
        /// <summary>True when values came from H750 binary 0xF0 / 0x21 frames.</summary>
        public bool IsBinaryMetrics { get; set; }
    ...
    public class STM32CommManager : IDisposable
    {
        ...
        private const int REPORT_TIMEOUT_MS = 15000;
        ...
        private const int H750_MAX_CLEN = 256;
        ...
        public async Task<bool> Connect(string portName)
        {
            ...
            try
            {
                ...
                bool pingOk = await Ping();
                if (!pingOk)
                    pingOk = await PingText();
                Log(pingOk
                    ? "Connected to " + portName + " — PING OK"
                    : "Connected to " + portName + " — PING failed (check H750 firmware / COM port)");
        ...
        // =====================================================================
        // H750 BINARY FRAMING (AA 55 | len | cmd | payload | CRC8 | 55 AA)
        // =====================================================================
        ...
        private static byte[] BuildH750Frame(byte cmd, byte[] payload)
        {
            int plen = payload?.Length ?? 0;
            int clen = 1 + plen;
            byte[] frame = new byte[clen + 7];
            int i = 0;
            frame[i++] = FRAME_H1;
            frame[i++] = FRAME_H2;
            frame[i++] = (byte)(clen & 0xFF);
            frame[i++] = (byte)((clen >> 8) & 0xFF);
            frame[i++] = cmd;
            if (plen > 0)
            {
                Array.Copy(payload, 0, frame, i, plen);
                i += plen;
            }
            frame[i++] = ComputeCrc8(frame, 2, clen + 2);
            frame[i++] = FRAME_F1;
            frame[i++] = FRAME_F2;
            return frame;
        }
        ...
        /// <summary>
        /// Consumes only auto-telemetry (0xF0) frames from the RX buffer.
        /// Command responses stay in the buffer for TryExtractPacket.
        /// </summary>
        private void ProcessBinaryTelemetryFrames()
        {
            lock (_rxBuffer)
            {
                int guard = 0;
                while (_rxBuffer.Count >= 7 && guard++ < 32)
                {
                    if (_rxBuffer[0] != FRAME_H1 || _rxBuffer[1] != FRAME_H2)
                    {
                        _rxBuffer.RemoveAt(0);
                        continue;
                    }

                    int clen = _rxBuffer[2] | (_rxBuffer[3] << 8);
                    if (clen < 1 || clen > H750_MAX_CLEN)
                    {
                        _rxBuffer.RemoveAt(0);
                        continue;
                    }
                    ...
                    if (_rxBuffer[4] != STM32Commands.CMD_TELEMETRY)
                        return;

                    byte[] frame = _rxBuffer.GetRange(0, frameLen).ToArray();
                    byte crcExpected = ComputeCrc8(frame, 2, clen + 2);
                    if (crcExpected != frame[4 + clen] ||
                        frame[frameLen - 2] != FRAME_F1 ||
                        frame[frameLen - 1] != FRAME_F2)
                    {
                        _rxBuffer.RemoveAt(0);
                        continue;
                    }
        ...
        private byte[] TryExtractPacket(byte expectedCmd)
        {
            lock (_rxBuffer)
            {
                ...
                while (_rxBuffer.Count >= 7 && guard++ < 32)
                {
                    ...
                    if (_rxBuffer.Count < frameLen)
                    {
                        return TryExtractLegacyPacket(expectedCmd);
                    }
                    ...
                    if (payload.Length > 0 && payload[0] == STM32Commands.CMD_TELEMETRY)
                    {
                        ParseTelemetryPayload(payload);
                        continue;
                    }
        ...
        /// <summary>Older firmware: AA 55 [cmd] [data...] 55 AA (no length/CRC).</summary>
        private byte[] TryExtractLegacyPacket(byte expectedCmd)
        {
            while (_rxBuffer.Count >= 6)
            {
                if (_rxBuffer[0] != FRAME_H1 || _rxBuffer[1] != FRAME_H2)
                {
                    _rxBuffer.RemoveAt(0);
        ...
        private void ParseTelemetryPayload(byte[] payload)
        {
            if (payload == null || payload.Length < 15 ||
                payload[0] != STM32Commands.CMD_TELEMETRY)
                return;
            ...
            try
            {
                ...
                var t = new TelemetryData
                {
                    ...
                    IsBinaryMetrics = true,
                    TotalRms = BitConverter.ToSingle(payload, idx)
                };
                idx += 4;
                t.FaultIndex = BitConverter.ToSingle(payload, idx);
                idx += 4;
                t.FaultLevel = payload[idx++];
                t.ImbalancePct = BitConverter.ToSingle(payload, idx);
                idx += 4;
                if (idx < payload.Length)
                    t.MotorStatus = payload[idx];
        ...
        /// <summary>
        /// Waits for multiple text lines (e.g., REPORT).
        /// Collects until end marker or idle timeout.
        /// </summary>
        private async Task<List<string>> WaitForMultiLineResponse(
            int timeoutMs, int idleMs = 300)
        {
        ...
        {
            byte[] pl = BitConverter.GetBytes(value);
            return await SendAndWaitAck(cmd, pl, CMD_TIMEOUT_MS);
        }

        private async Task<bool> SendUint16Command(byte cmd, ushort value)
        {
            byte[] pl = { (byte)(value & 0xFF), (byte)((value >> 8) & 0xFF) };
            return await SendAndWaitAck(cmd, pl, CMD_TIMEOUT_MS);
        }

        private async Task<bool> SendSimpleCommand(byte cmd, int timeout)
        {
            return await SendAndWaitAck(cmd, null, timeout);
        }

        private async Task<bool> SendAndWaitAck(byte expectedCmd,
            byte[] payload, int timeout)
        {
            try
            {
                lock (_rxBuffer) { _rxBuffer.Clear(); }
                SendRaw(BuildH750Frame(expectedCmd, payload));
        ...
        private async Task<byte[]> SendAndWaitResponse(byte cmd, int timeout)
        {
            ...
            {
                lock (_rxBuffer) { _rxBuffer.Clear(); }
                SendRaw(BuildH750Frame(cmd, null));
        ...
        {
            ushort rpm = (ushort)Math.Max(100, Math.Min(30000, (int)Math.Round(v)));
            return await SendUint16Command(STM32Commands.CMD_SET_RPM, rpm);
        }

        public async Task<bool> SetLineFrequency(float hz) =>
            await SendFloatCommand(STM32Commands.CMD_SET_LINE_FREQ, hz);
        ...
        {
            int ok = 0;
            if (p.SupplyLineHz > 10f && p.SupplyLineHz < 400f)
            {
                if (await SetLineFrequency(p.SupplyLineHz)) ok++;
        ...
        public async Task<MotorParameters> GetAllParameters()
        {
            ...
            if (response == null)
            {
                Log("GetAllParameters: no response (check H750 USB frame / COM port)");
                return null;
            }
            ...
            try
            {
                ...
                p.RatedCurrent = BitConverter.ToSingle(response, idx); idx += 4;
                p.RatedPower = BitConverter.ToSingle(response, idx); idx += 4;
                p.SupplyLineHz = BitConverter.ToSingle(response, idx); idx += 4;
                p.MotorPoles = (ushort)(response[idx] | (response[idx + 1] << 8));
                idx += 2;
                p.MotorRPM = (ushort)(response[idx] | (response[idx + 1] << 8));
                idx += 2;
        ...
        public async Task<TelemetryData> GetMetrics()
        {
            byte[] response = await SendAndWaitResponse(
                STM32Commands.CMD_GET_METRICS, CMD_TIMEOUT_MS);
            ...
            try
            {
                ...
                t.RmsA = BitConverter.ToSingle(response, idx); idx += 4;
                t.RmsB = BitConverter.ToSingle(response, idx); idx += 4;
                t.RmsC = BitConverter.ToSingle(response, idx); idx += 4;
                t.TotalRms = BitConverter.ToSingle(response, idx); idx += 4;
                t.FaultIndex = BitConverter.ToSingle(response, idx); idx += 4;
                t.ImbalancePct = BitConverter.ToSingle(response, idx); idx += 4;
                t.FaultLevel = response[idx++];
                t.MotorStatus = response[idx];
                return t;
            }
            catch (Exception ex)
            {
        ...
        public Task<bool> SaveAdcSnapshot()
        {
            Log("H750 firmware logs raw ADC automatically (raw_log_interval_s). No on-demand SAVEADC in binary mode.");
        ...
        /// <summary>
        /// Sends the REPORT command and waits for the full multi-line response.
        /// </summary>
        public async Task<List<string>> RequestReport()
        {
            try
            {
                _commandPending = true;
                ...
                List<string> lines = await WaitForMultiLineResponse(REPORT_TIMEOUT_MS);

                _commandPending = false;
                Log("REPORT: received " + lines.Count + " lines");
                return lines;
            }
            catch (Exception ex)
            {
